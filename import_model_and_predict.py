...
def predict(text, model, tokenizer, device):
    ...
    with torch.no_grad():
        evidence_logits, suggestion_logits, connection_logits = model(input_ids=input_ids, attention_mask=attention_mask)
...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        text = row['Text']
        evidence_pred, suggestion_pred, connection_pred = predict(text, model, tokenizer, device)
